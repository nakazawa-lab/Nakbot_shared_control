...
#- TODO -#
def draw_robot_radius(ax,r,x,y,interval,line_width):
    for i in range(0,len(x),interval):
        c = patches.Circle(xy=(x[i], y[i]), radius=r,fill=False,ec="blue",linewidth=line_width)
        ax.add_patch(c)
...
def plot_start_goal(x_start,y_start,x_goal,y_goal):
    ...
    plt.plot(gx, gy, color="red",linewidth=0.8)
    plt.plot(sx, sy, color="blue",linewidth=0.8)
